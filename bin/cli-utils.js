...
Utils.confirmationId = function(pocketeer) {
  return parseInt(pocketeer.xPubKeySignature.substr(-4), 16).toString().substr(-4);
}
...
Utils.getClient = function(args, opts, cb) {
  ...
  var host = args.host || process.env['APWS_HOST'] || 'https://apocket.network/';
  ...
  var client = new Client({
    baseUrl: url.resolve(host, '/apws/api'),
  ...
  _.each(txps, function(x) {
    ...
    if (!_.isEmpty(x.actions)) {
      console.log('\t\tActions: ', _.map(x.actions, function(a) {
        return a.pocketeerName + ' ' + (a.type == 'accept' ? '✓' : '✗') + (a.comment ? ' (' + a.comment + ')' : '');
      }).join('. '));
    }
